...
public interface UserLoanEMIRepository extends JpaRepository<LoanEMIUser, Long>{
	...
	@Query(nativeQuery = true,
		       value ="SELECT * FROM railway.loan_emi WHERE user_id=?1 ORDER BY emi_payment_date DESC LIMIT 1")
	...
	@Query(nativeQuery = true,
		       value ="SELECT * FROM railway.loan_emi WHERE user_id=?1 AND DATE(next_emi_date) <= CURDATE() ORDER BY next_emi_date DESC LIMIT 1")
